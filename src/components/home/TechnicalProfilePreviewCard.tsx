...
export default function TechnicalProfilePreviewCard() {
  ...
  return (
    <CardContainer>
      {/* HEADER: Mais respiro vertical (pb-3, mb-6) */}
      <View className="flex-row items-center justify-between mb-4 border-b border-slate-800/50 pb-3">
        ...
        <Pressable 
          ...
          className="flex-row items-center gap-1 active:opacity-60 px-2 py-1 -mr-2"
        >
          <Text className="text-[10px] font-medium text-amber-500 uppercase">Editar</Text>
          <Ionicons name="pencil-sharp" size={10} color="#fbbf24" />
        </Pressable>
      </View>

      {/* BLOCO 1: CONDUTOR */}
      {/* mb-8 cria a "Divisão Invisível" grande antes do veículo */}
      <View className="mb-4 pl-1">
        <View className="flex-row justify-between items-start">
          
          {/* Dados Pessoais */}
          <View className="flex-1 mr-4">
            {/* mb-1.5 separa o label do Nome */}
            ...
            <Text className="text-x text-amber-500 font-mono">
              CPF ***.{profile.driver.cpf.slice(3,6)}.{profile.driver.cpf.slice(6,9)}-**
            </Text>
          </View>
      ...
      {/* BLOCO 2: VEÍCULO */}
      {/* p-4 aumenta o conforto visual interno do card escuro */}
      <View className="bg-slate-900/80 rounded-lg p-4 border border-slate-800 flex-row items-center gap-4">
        ...
        <View className="flex-1 justify-center">
          ...
          <View className="flex-row items-center gap-2">
            ...
            <Text className="text-[10px] text-slate-500 flex-1" numberOfLines={1}>
